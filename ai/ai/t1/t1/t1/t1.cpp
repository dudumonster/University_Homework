...
typedef struct Node
{
	status data;//结点所存储的状态 ，一个 3*3 矩阵
	struct Node* parent;//指向结点的父亲结点

	struct SpringLink* child;//指向结点的后继结点
	struct Node* next;//指向 open 或者 closed 表中的后一个结点
	int fvalue;//结点的总的路径
	int gvalue;//结点的实际路径
...
//开始状态与目标状态
/*
status startt = {1,3,0,8,2,4,7,6,5};最佳路径为 2
status startt = {1,3,0,2,8,4,7,6,5};迭代超过 20000 次，手动停止
...
void AStar()
{
	...
	PNode tmpNode = NULL;//指向从 open 表中拿出并放到 closed 表中的结点的指针
	PNode spring;//tmpNode 的后继结点链
	PNode tmpLNode;//tmpNode 的某一个后继结点
	PNode tmpChartNode;

	PNode thePreNode;//指向将要从 closed 表中移到 open 表中的结点的前一个结点的指针
	bool getGoal = false;//标识是否达到目标状态
	long numcount = 1;//记录从 open 表中拿出结点的序号
	...
	cout << "1" << endl;
	cout << "从 open 表中拿出的结点的状态及相应的值" << endl;

	while (!isEmpty(open))
	{
		t++;
		//从 open 表中拿出 f 值最小的元素,并将拿出的元素放入 closed 表中
		...
		cout << "其 f 值为：" << tmpNode->fvalue << endl;
		cout << "其 g 值为：" << tmpNode->gvalue << endl;
		cout << "其 h 值为：" << tmpNode->hvalue << endl;
		...
		//产生当前检测结点的后继(与祖先不同)结点列表，产生的后继结点的 parent 属性指向当前检测的结点
		...
		while (!isEmpty(spring))
		{
			...
			//状态在 open 表中已经存在，thePreNode 参数在这里并不起作用
			...
			//新的状态即此状态既不在 open 表中也不在 closed 表中
			else
			{
